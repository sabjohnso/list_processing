...
namespace ListProcessing::Dynamic::Details
{
  ...
  public:
     ...
     */
    friend bool
    hasData(Stream xs){ return xs.hasData(); };
     ...
     */
    friend bool
    isEmpty(Stream xs) { return xs.isEmpty(); }
    ...
    append(Stream xs) const {
      return hasData()
        ? (xs.hasData()
           ? Stream([=,this]{ return pair(xs.head(), append(xs.tail())); })
           : *this)
        : xs;
    }
    ...
    //            |_|
    /**
     * @brief Return a Stream with the input function mapped over this stream.
     */
    template<typename G, typename U = decay_t<result_of_t<G(T)>>>
    Stream<U>
    map(G g) const {
      return hasData()
        ? Stream<U>([=,xs = *this]{ return pair(g(xs.head()), xs.tail().map(g)); })
        : Stream<U>();
    }
     ...
     */
    friend size_type
    length(Stream xs){ return xs.length(); }
    ...
    value_type
    listRef(index_type i) const { return streamRef(i); }
  ...
  private:
    ...
    public:
      Kernel(Kernel&& input) : data(move(input.data)){}
  ...
  private:
    ...
    aux(size_type n, F&& f, index_type i){
      return i < n
        ? Stream<U>([=]{ return pair<U,Stream<U>>(f(i), aux(n, f, i+1)); })
        : empty_stream<U>;
    }
