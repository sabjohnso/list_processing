...
namespace ListProcessing::CompileTime::Details {
  ...
  public:
    ...
    erase() const
    {
      return constructTape(tail(data), context);
    }
    ...
    fwd() const
    {
      if constexpr (length_(type<D>) == 0) {
        ...
        return constructTape(tail(data), cons(head(data), context));
      }
    }
    ...
    bwd() const
    {
      if constexpr (length_(type<C>) == 0) {
        ...
        return constructTape(cons(head(context), data), tail(context));
      }
    }
    ...
    constexpr auto moveBy(integral_constant<offset_type, N>) const
    {
      if constexpr (N > 0) {
    ...
    constexpr auto moveTo(integral_constant<index_type, N>) const
    {
  ...
  template<typename T>
  struct IsTape
    : conditional_t<is_same_v<T, decay_t<T>>, false_type, IsTape<decay_t<T>>>
  {};
